...
const monthsBetween = (start: string | undefined) => {
  if (!start) return 0;
...
export const calcPrimeAnciennete = (
  base: number,
  annees: number,
  grille: AtlasData['settings']['payrollParams']['prime_anciennete']
) => {
...
export const calcHeuresSup = (
  hNorm = 0,
  h25 = 0,
  h50 = 0,
  h100 = 0,
  tauxHoraire = 0,
  config: Partial<AtlasData['settings']['payrollParams']['overtime']> = {}
) => {
...
export const calcCNSS = (
  brut: number,
  params: AtlasData['settings']['payrollParams']
) => {
  const { cnss } = params;
...
export const calcBaseImposable = (brut: number, retenuesCNSS: number, abattements = 0) => {
...
export const calcIR = (
  baseIR: number,
  bareme: AtlasData['settings']['payrollParams']['ir_bareme_2025']
) => {
  let restant = baseIR;
  let impots = 0;
  let previousMax = 0;
...
export const calcNetAPayer = (brut: number, totalRetenues: number, ir: number) => {
...
type EmployeeVariables = Record<string, PayrollVariable>;
...
type PayrollResult = {
  ...
  primes: number;
  brut: number;
  cnss: ReturnType<typeof calcCNSS>;
